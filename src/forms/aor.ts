import { VOWELS, CONSONANTS } from "../deps.ts";
...
Object.defineProperty(String.prototype, "insertAt", {
    value: function(idx, val) {
      return idx > 0
        ? this.slice(0, idx) + val + this.slice(idx)
        : val + this;
    }
});
...
export function getRootAor(root, { root_srs2, thema, person_s }) {
  ...
  // TODO: can have only single match? use relace instead
    ...
    : root_srs2 == "R2" && thema == null && root.includes("ე") && root.endsWith("ევ")
    ...
    ? person_s.endsWith("3")
      ? root
      : root.insertBeforeLast(CONSONANTS, "ე")

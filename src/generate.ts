...
function getGRP1(args, exceptions) {
  ...
  const { PRS: excPRS, IMPF: excIMPF, PRSSUBJ: excPRSSUBJ, ...excAll } =
    excRest;
...
function getGRP2(args, exceptions) {
  ...
  const { FUT: excFUT, COND: excCOND, FUTSUBJ: excFUTSUBJ, ...excAll } =
    excRest;
...
function getForm(args, exceptions, form, person_s, person_o) {
  ...
  const { preverb, person1, version, root, thema, modus, perfect2, person2 } =
    form(args, person_s, person_o);
